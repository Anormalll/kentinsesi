    ...
    <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-zinc-200 dark:border-zinc-800">
        <div className="flex justify-between items-center mb-6">
            <h2 className="font-bold text-lg text-zinc-900 dark:text-white">Kurumsal Araç Kaydı</h2>
            <button onClick={onClose} className="text-zinc-500 dark:text-zinc-400"><X /></button>
        </div>
        <div className="space-y-4">
            <div>
                <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase">Plaka</label>
                <input value={plate} onChange={handlePlateChange} placeholder="34AB1234" className={`w-full h-12 border-2 rounded-xl px-4 font-mono text-lg uppercase bg-white dark:bg-zinc-950 dark:text-white ${isValid ? 'border-green-500 bg-green-50 dark:bg-green-900/10' : 'border-zinc-200 dark:border-zinc-700'}`} />
                {!isValid && plate.length > 0 && <p className="text-[10px] text-red-500 mt-1">Geçersiz Plaka Formatı</p>}
            </div>
            <div>
                <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase">Ruhsat Seri / No</label>
                <input value={serialNo} onChange={e => setSerialNo(e.target.value)} placeholder="AB123456" className="w-full h-12 border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 dark:text-white rounded-xl px-4" />
            </div>
            <button disabled={!isValid || !serialNo || loading} onClick={handleSubmit} className="w-full h-12 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 font-bold rounded-xl disabled:opacity-50 flex items-center justify-center">
  ...
  const renderContent = () => {
    ...
    if (role === 'VATANDAS') {
        if (view === 'HOME') return (
            <div className="p-6 flex flex-col min-h-[80vh]">
                <div className="mb-8">
                    <button onClick={() => setIsReportModalOpen(true)} className="w-full h-24 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-2xl shadow-xl shadow-red-200 dark:shadow-none flex items-center justify-center gap-4 font-black text-2xl active:scale-[0.98] transition-transform">
                    ...
                    <div className="space-y-3">
                        {complaints.filter(c => c.isMyReport).slice(0, 5).map(c => (
                            <div key={c.id} onClick={() => setSelectedComplaint(c)} className="bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800 flex items-center gap-3 shadow-sm active:bg-zinc-50 dark:active:bg-zinc-800 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-sm text-zinc-800 dark:text-zinc-100 truncate">{c.title}</h4>
        ...
        if (view === 'LISTE') return (
            <div className="p-4 space-y-4">
                <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Bildirim Geçmişi</h2>
                {complaints.filter(c => c.isMyReport).length === 0 ? <div className="text-center text-zinc-400 py-10">Henüz bildiriminiz yok.</div> :
                 complaints.filter(c => c.isMyReport).map(c => (
                    <div key={c.id} onClick={() => setSelectedComplaint(c)} className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex gap-4 shadow-sm relative group">
                        <img src={c.image} className="w-16 h-16 rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" onError={(e) => { e.currentTarget.src = 'https://placehold.co/100?text=Resim'; }} />
                        <div className="flex-1 min-w-0">
                            ...
                            <h4 className="font-bold text-zinc-900 dark:text-zinc-100 truncate">{c.title}</h4>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate mt-1">{c.plate || 'Plaka Yok'}</p>
                        </div>
                    </div>
                 ))}
            </div>
    ...
    if (role === 'BELEDIYE_YETKILISI' && view === 'DASHBOARD') return (
        <div className="p-4 space-y-6">
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Yönetim Paneli</h2>
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm"><div className="text-zinc-400 text-xs font-bold uppercase">Bekleyen</div><div className="text-3xl font-black text-red-600">{stats.allTotal - stats.allResolved}</div></div>
                <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm"><div className="text-zinc-400 text-xs font-bold uppercase">Çözülen</div><div className="text-3xl font-black text-emerald-500">{stats.allResolved}</div></div>
            </div>
            
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h3 className="font-bold text-zinc-900 dark:text-white">Kurumsal Filo</h3>
                    <button onClick={() => setIsVehicleModalOpen(true)} className="bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 active:scale-95"><Plus size={14} /> Ekle</button>
                </div>
                ...
                {vehicles.length === 0 ? <div className="text-center text-zinc-400 text-sm py-4 border border-dashed rounded-xl">Kayıtlı araç yok.</div> : (
                    <div className="grid gap-3">
                        {vehicles.map(v => {
                            ...
                            return (
                                <div key={v.id} 
                                     onClick={() => setInspectingPlate(v.plate)}
                                     className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center justify-between cursor-pointer hover:border-zinc-400 transition-colors active:bg-zinc-50 dark:active:bg-zinc-800">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full flex items-center justify-center"><Car size={20} /></div>
                                        <div>
                                            <p className="font-black font-mono text-lg text-zinc-800 dark:text-zinc-100">{v.plate}</p>
                                            <p className="text-[10px] text-zinc-400">Ruhsat: {v.serial_no}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className={`flex flex-col items-center px-3 py-1 rounded-lg ${complaintCount > 0 ? 'bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400'}`}>
    ...
    if (role === 'BELEDIYE_YETKILISI' && view === 'LISTE') return (
        <div className="p-4 space-y-4">
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Tüm Bildirimler</h2>
            {complaints.map(c => (
                <div key={c.id} onClick={() => setSelectedComplaint(c)} className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex gap-4 shadow-sm relative"><div className="flex-1"><div className="flex justify-between mb-1"><StatusBadge status={c.status} /></div><h4 className="font-bold text-sm truncate dark:text-white">{c.title}</h4></div><ChevronRight className="text-zinc-300 self-center" /></div>
            ))}
        </div>
    );
    ...
    if (view === 'PROFIL') return (
        <div className="p-4 space-y-6">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 flex flex-col items-center text-center shadow-sm">
                ...
                <h2 className="text-xl font-bold text-zinc-900 dark:text-white">{role === 'VATANDAS' ? 'Vatandaş Hesabı' : 'Firma Yetkilisi'}</h2>
            </div>

            {role === 'VATANDAS' && (
                <div className="grid grid-cols-3 gap-3">
                    <div className="bg-white dark:bg-zinc-900 p-3 rounded-2xl border border-zinc-100 dark:border-zinc-800 text-center shadow-sm">
    ...
    <div className="max-w-md mx-auto bg-zinc-50 dark:bg-zinc-950 min-h-screen flex flex-col shadow-2xl relative transition-colors duration-300">
      <header className="h-16 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 bg-white dark:bg-zinc-900 sticky top-0 z-10 transition-colors duration-300">
        <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-zinc-900 dark:text-white tracking-tight text-lg">Hatalısın</span>
        </div>
        <div className="flex items-center gap-2">
            {role && (<div className="text-[10px] font-bold text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-full border border-zinc-200 dark:border-zinc-700 flex items-center gap-1"><Globe size={10} /> {role === 'VATANDAS' ? 'İSTANBUL' : 'YÖNETİM'}</div>)}
            {/* KARANLIK MOD BUTONU */}
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
      ...
      {inspectingPlate && (
        <div className="fixed inset-0 z-[105] bg-white dark:bg-zinc-900 flex flex-col animate-in slide-in-from-right duration-300">
             <div className="h-16 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sticky top-0 bg-white dark:bg-zinc-900 shadow-sm">
                <button onClick={() => setInspectingPlate(null)} className="p-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full"><ArrowLeft /></button>
                <span className="font-bold text-lg font-mono text-zinc-900 dark:text-white">{inspectingPlate}</span>
                <div className="w-10" />
             </div>
             <div className="flex-1 overflow-y-auto p-4 space-y-3">
                 ...
                 {complaints.filter(c => c.plate === inspectingPlate).length === 0 ? 
                    <div className="text-center text-zinc-400 py-10 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl">Kayıtlı ihlal bulunamadı.</div> :
                    complaints.filter(c => c.plate === inspectingPlate).map(c => (
                        <div key={c.id} onClick={() => setSelectedComplaint(c)} className="bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 flex items-center gap-3 shadow-sm active:bg-zinc-50 dark:active:bg-zinc-800 cursor-pointer">
                            <StatusBadge status={c.status} />
                            <div className="flex-1 min-w-0">
                                <h4 className="font-bold text-sm text-zinc-900 dark:text-zinc-100 truncate">{c.title}</h4>
      ...
      {selectedComplaint && (
        <div className="fixed inset-0 z-[110] bg-white dark:bg-zinc-900 flex flex-col animate-in slide-in-from-right duration-300">
             <div className="h-16 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sticky top-0 bg-white dark:bg-zinc-900 shadow-sm"><button onClick={() => setSelectedComplaint(null)} className="p-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full"><ArrowLeft /></button><span className="text-xs font-bold text-zinc-500 dark:text-zinc-400">DETAY #{selectedComplaint.id.slice(0,4)}</span>
             {selectedComplaint.isMyReport && <button onClick={() => handleDelete(selectedComplaint.id)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full"><Trash2 size={18}/></button>}
             </div>
            <div className="flex-1 overflow-y-auto">
                <div className="bg-zinc-100 dark:bg-zinc-950 w-full flex justify-center py-4">
                ...
                <div className="p-5 space-y-4">
                    <div className="flex items-center justify-between">
                         <h1 className="text-xl font-bold text-zinc-900 dark:text-white">{selectedComplaint.title}</h1>
                         <StatusBadge status={selectedComplaint.status} />
                    </div>
                    <p className="text-zinc-600 dark:text-zinc-300 text-sm leading-relaxed">{selectedComplaint.description}</p>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-zinc-50 dark:bg-zinc-800 p-3 rounded-xl border border-zinc-100 dark:border-zinc-700"><p className="text-[10px] font-bold text-zinc-400 uppercase mb-1">PLAKA</p><p className="font-mono text-lg font-bold text-zinc-800 dark:text-zinc-100">{selectedComplaint.plate || '-'}</p></div>
                        <div className="bg-zinc-50 dark:bg-zinc-800 p-3 rounded-xl border border-zinc-100 dark:border-zinc-700"><p className="text-[10px] font-bold text-zinc-400 uppercase mb-1">KONUM</p><p className="text-xs text-zinc-800 dark:text-zinc-100 line-clamp-2">{selectedComplaint.location}</p></div>
                    </div>
                        ...
                        <div className="mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                ...
                                <button 
                                    ...
                                    className={`py-3 rounded-xl text-xs font-bold transition-all border-2 ${selectedComplaint.status === 'İşleniyor' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-zinc-800 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900 hover:border-blue-300'}`}
                                >
                                    İŞLENİYOR
                                </button>
                                <button 
                                    onClick={() => handleUpdateStatus(selectedComplaint.id, 'Çözüldü')}
                                    className={`py-3 rounded-xl text-xs font-bold transition-all border-2 ${selectedComplaint.status === 'Çözüldü' ? 'bg-emerald-500 text-white border-emerald-500' : 'bg-white dark:bg-zinc-800 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900 hover:border-emerald-300'}`}
                                >
                                    ÇÖZÜLDÜ
                                </button>
                            </div>
